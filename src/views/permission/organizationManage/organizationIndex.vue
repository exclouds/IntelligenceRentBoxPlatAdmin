<template>
  <el-container class="orgMng">
    ...
    <el-main class="height:100%">
      <el-header style="height: 30px">
        ...
        <el-button
          type="primary"
          size="small"
          @click="openInsertOrganization()"
          v-if="checkBtnPeimission(orgPage.addOrg.permission)"
          >{{ orgPage.addOrg.name }}</el-button
        >
        <el-button
          type="primary"
          size="small"
          @click="openUpdateOrganization()"
          v-if="checkBtnPeimission(orgPage.updateOrg.permission)"
          >{{ orgPage.updateOrg.name }}</el-button
        >
        <el-button
          type="primary"
          size="small"
          @click="deleteOrganization()"
          v-if="checkBtnPeimission(orgPage.deleteOrg.permission)"
          >{{ orgPage.deleteOrg.name }}</el-button
        >
      </el-header>
      <el-main style="height: calc(100% - 30px)">
        ...
        <el-row
          style="height: calc(100% - 40px); width: 99%; margin-top: -20px"
        >
          <depinfoList
            ref="depComp"
            v-show="tab === '1'"
    ...
    <edit-organization
      ref="editOrganizatonComp"
      :pshow="editOrganizationComp.show"
      @on-show-change="closeOrgCompShowChange"
      @on-save-success="onSaveSuccess"
    ></edit-organization>
  </el-container>
</template>
...
export default {
  ...
  methods: {
    ...
    openUpdateOrganization() {
      if (
        this.$refs.tree.$refs.tree.getCheckedNodes(false, false).length === 0
      ) {
        warnMsg("请选择要修改的公司");
        return;
      }
      ...
      this.$refs.editOrganizatonComp.getOrgDetailById(
        this.$refs.tree.$refs.tree.getCheckedNodes(false, false)[0].nodeId
      );
    },
    //删除组织机构
    deleteOrganization() {
      if (
        this.$refs.tree.$refs.tree.getCheckedNodes(true, false).length === 0
      ) {
        warnMsg("请选择要删除的公司");
        return;
      }
      this.$confirm("是否删除该公司?", "提示", {
        ...
        .then(() => {
          deleteOrganizaton({
            id: this.$refs.tree.$refs.tree.getCheckedNodes(true, false)[0]
              .nodeId,
          }).then((res) => {
            if (res.success) this.onSaveSuccess();
          });
        })
        .catch(() => {});
    },
    ...
    tabChange(tab) {
      if (
        this.treeComp.currNodeID !== "" &&
        this.treeComp.currNodeID !== null
      ) {
        if (tab.name === "1") {
          this.$refs.depComp.Id = this.treeComp.currNodeID;
          this.$refs.depComp.code = this.treeComp.currcode;
