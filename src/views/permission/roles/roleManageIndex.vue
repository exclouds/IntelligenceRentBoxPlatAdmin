<template>
  <div class="app-container">
          ...
          <el-table-column align="left" label="角色名称" min-width="50%">
            <template slot-scope="scope">
              <span v-if="scope.row.isSet">
                <el-form-item
                  :prop="'data.' + scope.$index + '.displayName'"
                  :rules="tableForm.rules.displayName"
                >
                  <el-input
                    size="mini"
                    maxlength="50"
                    v-model.trim="scope.row.displayName"
                  ></el-input>
                </el-form-item>
              </span>
    ...
    <role-auth
      ref="roleAuth"
      :pshow="roleAuthComp.show"
      :pRoleID="roleAuthComp.roleId"
      @on-show-change="onRoleAuthShowChange"
    ></role-auth>
  </div>
</template>
