  ...
  methods: {
    ...
    setComBox() {
      getDicListByDitType({
        ...
        var arr = { value: "ALL", displayText: "全部" };
        this.typeList.push(arr);
        //this.typeList.push = res.result.comboxs;
        this.typeList.push.apply(this.typeList, res.result.comboxs);
        this.InsidetypeList = res.result.comboxs;
      });
      GetRegionList({ IsAuthority: false, IsShowAll: true }).then((res) => {
        this.regionList = res.result;
      });

      getDicListByDitType({
        BaseKeyValueTypeCode: "INSIDEBU",
        Code: "",
        Name: "",
      }).then((res) => {
        this.InsidetypeList.push.apply(this.InsidetypeList, res.result.comboxs);
      });
    },
    //单击树选择框控件
