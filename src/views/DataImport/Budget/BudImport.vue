<template>
    <div class="app-container budimport" v-loading="divLoading">
...
export default {
    ...
    methods: {
       getRowKeys(row) {
        ...
        getOrganizationTree() {
            //{IsShowdep:true}
            getOrganizatonTreeNoUsers({IsByHZ:false}).then(res => {
            if(res.success)
      ...
      importaccount(file) {       
        ...
        this.importFile(file, (data) => {
            ...
            data[0].parseData.forEach((arrItem,i) => {
                             ...
                             //contrast[index] && (obj[contrast[index]] = amt || '')
                                ...
                                {  
                                    ...
                                    if(num[1].length>2)
                                    {
                                        ...
                                        // var newamt=Math.round(Number(item) * 1000) / 1000;;
                                        // var newnum=newamt.toString().split(".");
                                       
                                        // if(Number(newnum[1]).toString().length>2)
                                        // {
                                        //      if(msg=="")
                                        //     {
                                        //         msg=showname;
                        ...
                        if(firno) {
                            ...
                            {              
                                ...
                                firfeename=obj.ParentProjectName;
                                    if(obj.ProjectName ==="" || obj.ProjectName ===null || obj.ProjectName ===undefined ){
                                        obj.ProjectName=obj.ParentProjectName;
                                        obj.ParentProjectName="";
                                    }
                                }
                                arr.push(obj);               
                            }   
                ...
                if(filemsg!=="")
                {
                     var msg=arrItem.name+"下的" +filemsg+"";
                     showmsg+=msg;
                }
    ...
    importFile(fileArr,  func) {
        ...
        Importdata() {
           ...
           {
               warnMsg("未找到预算数据，请先导入预算！");
                return;
           }
           var budlist=[];
           this.TableDataList.forEach(item=>{
                if(item.data.filter(items=>items.LinNO==="" || items.LinNO===null || items.LinNO===undefined).length>0)
                {
