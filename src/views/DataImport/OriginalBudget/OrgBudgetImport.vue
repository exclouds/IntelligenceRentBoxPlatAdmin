...
import {ImportOrgReport,CheckImportOrgReport} from 'api/DataImport/OriginalBudget'
          ...
          let contrast = {
            ...
            this.importFile(file, (data) => {
              ...
              data[0].parseData.forEach((arrItem,i) => {
                   var filemsg=[];
                if(arrItem.name==="利润表"  || arrItem.name==="费用表"){
                            ...
                            if(firno && mindex <lastrow) {
                              ...
                              for(let [index,item] of dataitem.entries()) {
                               // var celldata=Number(item.toString());
                                ...
                                if(index>7 && mindex>(firrowno+1) && item !=="" && item !=null && item !==undefined && item !=="-")
                                {       
                      ...
                      title1.forEach((item,index)=>{
                       ...
                       if(filemsg.length>0)
                      {
                        var nowmsg=arrItem.name+"下的"+"<br>";
                        var alllino=filemsg.map(item => {return item.LinNO; });
                        nowmsg+="第"+alllino.toString() + "行数据";
                        // filemsg.forEach(tt =>{
                ...
                if(showmsg==="")
                {
                    ...
                    this.TableDataList=[];
                    showmsg="以下行号存在小数点超过两位的数据，请数据重新核算后导入：<br>"+showmsg;
            ...
            if(budlist.length>0)
           {
            ...
            let data={
             ...
             this.bthloading=true;
              CheckImportOrgReport(data).then(res => {
                if(res.success)
                {
