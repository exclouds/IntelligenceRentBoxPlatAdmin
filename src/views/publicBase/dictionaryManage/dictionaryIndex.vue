<template>
  <el-container class="app-container dicMng">
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="11" style="margin-left: 20px">
            <el-button
              ...
              >搜索</el-button
            >
            <el-button type="primary" size="mini" @click="addDicType()"
              >新增类型</el-button
            >
            <el-button type="primary" size="mini" @click="editDicType()"
              >编辑类型</el-button
            >
            <el-button type="primary" size="mini" @click="deleteDicType()"
              >删除类型</el-button
            >
          </el-col>
        </el-row>
        ...
        <el-row style="height: calc(100% - 80px); margin-top: 10px">
          ...
          >
            ...
            >
              ...
              >
                <template slot-scope="scope">{{
                  scope.row.systemSetting ? "系统" : "自定义"
                }}</template>
              </el-table-column>
              <el-table-column
                align="center"
              ...
              <el-table-column align="center" label="操作" min-width="20%">
                <template slot-scope="scope" style="align-text: center">
                  ...
                  <div
                    class="tableBtn"
                    v-else
                    @click="onClick(scope.row, scope.$index, false)"
                  >
                    取消
                  </div>
                </template>
    ...
    <edit-dictionary-type
      ref="editDicTypeComp"
      :pshow="editDicComp.show"
      @on-show-change="onEditDicTypeCompShowChange"
      @on-save-success="getDictionaryTypeTree"
    ></edit-dictionary-type>
  </el-container>
</template>
<style lang="scss">
...
import {
  getDicTypeTree,
  getDicList,
  createOrUpdateDicItem,
  deleteDicType,
  deleteDicItem,
} from "api/publicBase/dictionaryMng";
  ...
  methods: {
    ...
    deleteDicType() {
      ...
      this.$confirm("是否删除该字典类型?", "提示", {
        ...
        .then(() => {
          deleteDicType({
            input: this.$refs.tree.$refs.tree.getCheckedNodes(true, false)[0]
              .id,
          }).then((res) => {
    ...
    editDicType() {
      ...
      this.editDicComp.show = true;
      this.$refs.editDicTypeComp.getOrganizationTree();
      this.$refs.editDicTypeComp.getSingleDicItem(
        this.$refs.tree.$refs.tree.getCheckedNodes(false, false)[0].id
      );
    },
    //获取字典类型树
