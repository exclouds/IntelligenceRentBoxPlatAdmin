<template>
  <div class="app-container AccountSpecialFee"  v-loading="bthloading">
    ...
    <el-row>
         <el-col :span="5">
           ...
           <el-col :span="4">
            <el-form-item label="部门：" prop="" label-width="70px">
         ...
         <el-col :span="3">
          ...
          <el-col :span="5">
            <el-form-item label="科目编码/名称："  label-width="120px">
              <el-input placeholder="项目"  style="width:100%;" v-model="search.SubjectCode" size="mini" clearable ></el-input>
            </el-form-item>
          </el-col>
        ...
        <el-col :span="5">
          <el-form-item label prop label-width="10px">
           <el-button type="primary" size="mini" @click="getTableList()">搜索</el-button>       
            <el-button type="primary" size="mini" @click="openAdd()" v-if="checkBtnPeimission(AccountSpecialFee.add.permission)">新增</el-button>
          ...
          <el-popover
            ...
            width="400"
            trigger="hover">
                   <el-button type="primary" size="mini" @click="onBatchDelete()"  v-if="checkBtnPeimission(AccountSpecialFee.delete.permission)">删除</el-button>
                  <el-button type="primary" size="mini" @click="SubmitAccountSpecialFee()"  v-if="checkBtnPeimission(AccountSpecialFee.Submitbud.permission)">提交</el-button>
                  <el-button type="primary" size="mini" @click="CancelSubmit()"  v-if="checkBtnPeimission(AccountSpecialFee.CancelSubmitbud.permission)">取消提交</el-button>
                  <el-button type="primary" size="mini" @click="AgreeCancelSubmit()"  v-if="checkBtnPeimission(AccountSpecialFee.AgreeCancelSubmitbud.permission)">同意取消提交</el-button>
          ...
          <el-popover
            ...
            width="480"
            trigger="hover">
                  <el-button type="primary" size="mini" @click="BathOperation(4)"  v-if="checkBtnPeimission(AccountSpecialFee.delete.permission)">批量删除</el-button>
                  <el-button type="primary" size="mini" @click="BathOperation(1)"  v-if="checkBtnPeimission(AccountSpecialFee.Submitbud.permission)">批量提交</el-button>
                  <el-button type="primary" size="mini" @click="BathOperation(2)"  v-if="checkBtnPeimission(AccountSpecialFee.CancelSubmitbud.permission)">批量取消提交</el-button>
                  <el-button type="primary" size="mini" @click="BathOperation(3)"  v-if="checkBtnPeimission(AccountSpecialFee.AgreeCancelSubmitbud.permission)">批量同意取消提交</el-button>
        ...
        <el-table-column
         ...
         <el-table-column align="center" label="操作" width="130" >
            <template slot-scope="scope"  v-if="scope.row.isSubmit!==true  && scope.row.isAllLock!==true && scope.row.isLock!==true">
                <div class="tableBtn"  @click="openUpdate(scope.row)"  v-if="checkBtnPeimission(AccountSpecialFee.update.permission)">编辑</div>
                <div class="tableBtn"  @click="deletesinge(scope.row)"  v-if="checkBtnPeimission(AccountSpecialFee.delete.permission)">删除</div>                      
            </template>
          </el-table-column>
...
import { GetAccountSpecialFeeQueryList,BathDeleteAccountSpecialFee,SubmitAccountSpecialFee,CancelSubmit,AgreeCancelSubmit,BathOperation } from "api/DataMaintenance/AccountSpecialFee";
import EditSpecialFee from  "./EditSpecialFee"
import {warnMsg,successMsg} from 'utils/messageBox'
import {checkBtnPeimission,AccountSpecialFee} from 'utils/btnRole'
...
export default {
  ...
  methods: {
    ...
    getTableList() {
       if(this.search.code==="" || this.search.code===null ||this.search.code===undefined)
      {
       ...
       this.$refs.table.clearSelection();
        GetAccountSpecialFeeQueryList(data).then(res => {
          if(res.success)
          {
    ...
    deletesinge(row){
       this.$confirm('是否删除该调整事项?', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(() => {
                BathDeleteAccountSpecialFee([row.id]).then(res => {
                    if(res.success){
                        this.getTableList();
    ...
    onBatchDelete() {
      ...
      this.$confirm("是否确定删除", "提示", {
        ...
        BathDeleteAccountSpecialFee(arr).then(res => {
    ...
    openUpdate(row){
      ...
      this.$refs.EditComp.GetAccountSpecialFeeById(row.id);
    },
    openAdd(){
      this.EditComp.show = true
      ...
      this.$confirm("是否确定提交", "提示", {
        ...
        this.bthloading=true;
        SubmitAccountSpecialFee(arr).then(res => {
           this.bthloading=false;
            if(res.result!="" && res.result!==null){
               warnMsg(res.result);
             }
