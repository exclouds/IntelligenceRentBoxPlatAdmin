<template>
  <div class="app-container InsideTransaction"  v-loading="bthloading">
        ...
        <el-col :span="5">
          <el-form-item label prop label-width="10px">
           <el-button type="primary" size="mini" @click="getTableList()">搜索</el-button>       
            <!-- <el-button type="primary" size="mini" @click="openAdd()" v-if="checkBtnPeimission(InsideTransaction.add.permission)">新增</el-button> -->
           ...
           <el-popover
            ...
            trigger="hover">
                  <el-button type="primary" size="mini" @click="onBatchDelete()"  v-if="checkBtnPeimission(InsideTransaction.delete.permission)">删除</el-button> 
                  <el-button type="primary" size="mini" @click="SubmitData()"  v-if="checkBtnPeimission(InsideTransaction.Submitbud.permission)">提交</el-button>
                  <el-button type="primary" size="mini" @click="CancelSubmit()"  v-if="checkBtnPeimission(InsideTransaction.CancelSubmitbud.permission)">取消提交</el-button>
                  <el-button type="primary" size="mini" @click="AgreeCancelSubmit()"  v-if="checkBtnPeimission(InsideTransaction.AgreeCancelSubmitbud.permission)">同意取消提交</el-button>
                  <el-button type="primary" size="mini" @click="InitInsideMonthData()"  v-if="checkBtnPeimission(InsideTransaction.update.permission)">初始化数据</el-button>
          ...
          <el-popover
            ...
            trigger="hover">
                  <el-button type="primary" size="mini" @click="BathOperation(4)"  v-if="checkBtnPeimission(InsideTransaction.delete.permission)">批量删除</el-button>
                  <el-button type="primary" size="mini" @click="BathOperation(1)"  v-if="checkBtnPeimission(InsideTransaction.Submitbud.permission)">批量提交</el-button>
                  <el-button type="primary" size="mini" @click="BathOperation(2)"  v-if="checkBtnPeimission(InsideTransaction.CancelSubmitbud.permission)">批量取消提交</el-button>
                  <el-button type="primary" size="mini" @click="BathOperation(3)"  v-if="checkBtnPeimission(InsideTransaction.AgreeCancelSubmitbud.permission)">批量同意取消提交</el-button>
                  <el-button type="primary" size="mini" @click="BathOperation(5)"  v-if="checkBtnPeimission(InsideTransaction.update.permission)">批量初始化数据</el-button>
                  
              <el-button slot="reference" type="primary" size="mini" plain>查询批量操作</el-button>
          </el-popover>
               <el-button type="primary" size="mini" @click="Importdata()" v-if="checkBtnPeimission(InsideTransaction.update.permission)">导入</el-button>   
          </el-form-item>
        </el-col>
      </el-row>
    </el-form> 
        ...
        <el-table-column
         ...
         <el-table-column align="center" label="操作" width="130" fixed="right">
            <template slot-scope="scope"   v-if="scope.row.isSubmit!==true  && scope.row.isAllLock!==true && scope.row.isLock!==true">
                <div class="tableBtn"  @click="openUpdate(scope.row)" v-if="checkBtnPeimission(InsideTransaction.update.permission)">编辑</div>
                <div class="tableBtn"  @click="deletesinge(scope.row)" 
                v-if="checkBtnPeimission(InsideTransaction.delete.permission) && scope.row.id !==''&& scope.row.id !==null && scope.row.id !==undefined">删除</div>                      
            </template>
          </el-table-column>
...
,InitInsideMonthData} from "api/DataMaintenance/InsideTransaction";
import EditInsideTransaction from  "./EditInsideTransaction"
import {warnMsg,successMsg} from 'utils/messageBox'
import {checkBtnPeimission,InsideTransaction} from 'utils/btnRole'
    ...
    setComBox() {
      GetInsideBuComboBox().then((res) => {
        ...
        if(res.result.length>0){
          this.search.PreparationUnit=res.result[0].value;
        }
      });
