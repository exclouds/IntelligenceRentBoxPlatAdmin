<template>
  <div class="app-container ReportSpecialFee"  v-loading="bthloading">
    ...
    <el-row>
         <el-col :span="6">
          ...
          <el-col :span="4">
            <el-form-item label="报表类别："  label-width="90px">
             <el-select
        ...
        <el-col :span="5">
          <el-form-item label prop label-width="10px">
           <el-button type="primary" size="mini" @click="getTableList()">搜索</el-button>             
            <el-button type="primary" size="mini" @click="openAdd()" v-if="checkBtnPeimission(ReportSpecialFee.add.permission)">新增</el-button>
          ...
          <el-popover
            ...
            width="400"
            trigger="hover">
                   <el-button type="primary" size="mini" @click="onBatchDelete()"  v-if="checkBtnPeimission(ReportSpecialFee.delete.permission)">删除</el-button>
                  <el-button type="primary" size="mini" @click="SubmitReportSpecialFee()"  v-if="checkBtnPeimission(ReportSpecialFee.Submitbud.permission)">提交</el-button>
                  <el-button type="primary" size="mini" @click="CancelSubmit()"  v-if="checkBtnPeimission(ReportSpecialFee.CancelSubmitbud.permission)">取消提交</el-button>
                  <el-button type="primary" size="mini" @click="AgreeCancelSubmit()"  v-if="checkBtnPeimission(ReportSpecialFee.AgreeCancelSubmitbud.permission)">同意取消提交</el-button>
          ...
          <el-popover
            ...
            width="480"
            trigger="hover">
                  <el-button type="primary" size="mini" @click="BathOperation(4)"  v-if="checkBtnPeimission(ReportSpecialFee.delete.permission)">批量删除</el-button>
                  <el-button type="primary" size="mini" @click="BathOperation(1)"  v-if="checkBtnPeimission(ReportSpecialFee.Submitbud.permission)">批量提交</el-button>
                  <el-button type="primary" size="mini" @click="BathOperation(2)"  v-if="checkBtnPeimission(ReportSpecialFee.CancelSubmitbud.permission)">批量取消提交</el-button>
                  <el-button type="primary" size="mini" @click="BathOperation(3)"  v-if="checkBtnPeimission(ReportSpecialFee.AgreeCancelSubmitbud.permission)">批量同意取消提交</el-button>
        ...
        >
         ...
         <el-table-column align="center" label="操作" width="130" >
            <template slot-scope="scope"  v-if="scope.row.isSubmit!==true  && scope.row.isAllLock!==true && scope.row.isLock!==true">
                <div class="tableBtn"  @click="openUpdate(scope.row)"  v-if="checkBtnPeimission(ReportSpecialFee.update.permission)">编辑</div>
                <div class="tableBtn"  @click="deletesinge(scope.row)"  v-if="checkBtnPeimission(ReportSpecialFee.delete.permission)">删除</div>                      
            </template>
          </el-table-column>
      ...
      ></el-pagination>
     <EditReportSpecialFee  ref="EditComp" :pshow="EditComp.show" @on-show-change="onEditCompShowChange"
...
import { GetReportSpecialFeeQueryList,BathDeleteReportSpecialFee,SubmitReportSpecialFee,CancelSubmit,AgreeCancelSubmit,BathOperation } from "api/DataMaintenance/ReportSpecialFee";
import EditReportSpecialFee from  "./EditReportSpecialFee"
import {warnMsg,successMsg} from 'utils/messageBox'
import {checkBtnPeimission,ReportSpecialFee} from 'utils/btnRole'

export default {
  name: "ReportSpecialFeeIndex",
  components: { treeSelect,
                tree,EditReportSpecialFee},
  mixins: [tableMixin],
  data() {
    return {
      ReportSpecialFee,
  ...
  methods: {
    ...
    getTableList() {
       if(this.search.code==="" || this.search.code===null ||this.search.code===undefined)
      {
       ...
       this.$refs.table.clearSelection();
        GetReportSpecialFeeQueryList(data).then(res => {
          if(res.success)
          {
    ...
    deletesinge(row){
       this.$confirm('是否删除该报表特殊费用?', '提示', {
                ...
                BathDeleteReportSpecialFee([row.id]).then(res => {
                    if(res.success)
                    {
    ...
    onBatchDelete() {
      ...
      this.$confirm("是否确定删除", "提示", {
        ...
        BathDeleteReportSpecialFee(arr).then(res => {
          if(res.success){
    ...
    openUpdate(row){
      ...
      this.$refs.EditComp.GetReportSpecialFeeById(row.id);
    },
    openAdd(){
      this.EditComp.show = true
      ...
      this.$confirm("是否确定提交", "提示", {
        ...
        this.bthloading=true;
        SubmitReportSpecialFee(arr).then(res => {
           this.bthloading=false;
            if(res.result!="" && res.result!==null && res.result !==undefined){
               warnMsg(res.result);
             }
