  ...
  <div class="app-container RegionalCost"  v-loading="bthloading">
    ...
    <el-row>
         <el-col :span="6">
          ...
          <el-col :span="4">
            <el-form-item label="对方公司：" prop="" style="position:relative;" >            
                   <el-select v-model="search.tocode"  
                            placeholder="请选择公司" 
                            style="width:100%"                             
                            filterable     >
        ...
        <el-col :span="5">
          <el-form-item label prop label-width="10px">
           <el-button type="primary" size="mini" @click="getTableList()">搜索</el-button>             
            <el-button type="primary" size="mini" @click="openAdd()" v-if="checkBtnPeimission(RegionalCost.add.permission)">新增</el-button>
          ...
          <el-popover
            ...
            width="480"
            trigger="hover">
                   <el-button type="primary" size="mini" @click="onBatchDelete()"  v-if="checkBtnPeimission(RegionalCost.delete.permission)">删除</el-button>
                  <el-button type="primary" size="mini" @click="SubmitRegionalCost()"  v-if="checkBtnPeimission(RegionalCost.Submitbud.permission)">提交</el-button>
                  <el-button type="primary" size="mini" @click="CancelSubmit()"  v-if="checkBtnPeimission(RegionalCost.CancelSubmitbud.permission)">取消提交</el-button>
                  <el-button type="primary" size="mini" @click="BackCancelSubmit()"  v-if="checkBtnPeimission(RegionalCost.CancelSubmitbud.permission)">撤回取消提交</el-button>
                  <el-button type="primary" size="mini" @click="AgreeCancelSubmit()"  v-if="checkBtnPeimission(RegionalCost.AgreeCancelSubmitbud.permission)">同意取消提交</el-button>
          ...
          <el-popover
            ...
            width="620"
            trigger="hover">
                  <el-button type="primary" size="mini" @click="BathOperation(4)"  v-if="checkBtnPeimission(RegionalCost.delete.permission)">批量删除</el-button>
                  <el-button type="primary" size="mini" @click="BathOperation(1)"  v-if="checkBtnPeimission(RegionalCost.Submitbud.permission)">批量提交</el-button>
                  <el-button type="primary" size="mini" @click="BathOperation(2)"  v-if="checkBtnPeimission(RegionalCost.CancelSubmitbud.permission)">批量取消提交</el-button>
                  <el-button type="primary" size="mini" @click="BathOperation(5)"  v-if="checkBtnPeimission(RegionalCost.CancelSubmitbud.permission)">批量撤回取消提交</el-button>
                  <el-button type="primary" size="mini" @click="BathOperation(3)"  v-if="checkBtnPeimission(RegionalCost.AgreeCancelSubmitbud.permission)">批量同意取消提交</el-button>
        ...
        >
         ...
         <el-table-column align="center" label="操作" width="130" >
            <template slot-scope="scope"  v-if="scope.row.isSubmit!==true  && scope.row.isAllLock!==true && scope.row.isLock!==true">
                <div class="tableBtn"  @click="openUpdate(scope.row)"  v-if="checkBtnPeimission(RegionalCost.update.permission)">编辑</div>
                <div class="tableBtn"  @click="deletesinge(scope.row)"  v-if="checkBtnPeimission(RegionalCost.delete.permission)">删除</div>                      
            </template>
          </el-table-column>
      ...
      ></el-pagination>
     <EditRegionalCost  ref="EditComp" :pshow="EditComp.show" @on-show-change="onEditCompShowChange"
...
import { GetRegionalCostQueryList,BathDeleteRegionalCost,
SubmitRegionalCost,CancelSubmit,AgreeCancelSubmit,BathOperation ,BackCancelSubmit} from "api/DataMaintenance/RegionalCost";
import EditRegionalCost from  "./EditRegionalCost"
import {warnMsg,successMsg} from 'utils/messageBox'
import {checkBtnPeimission,RegionalCost} from 'utils/btnRole'

export default {
  name: "RegionalCostIndex",
  components: { treeSelect,
                tree,EditRegionalCost},
  mixins: [tableMixin],
  data() {
    return {
      RegionalCost,
  ...
  methods: {
      ...
      //获取组织树
        getOrganizationTree() {
            ...
            if(res.success)
                ...
                {
                  this.search.code=res.result[0].nodeUUid;
                }
            })
    ...
    getTableList() {
       if(this.search.code==="" || this.search.code===null ||this.search.code===undefined)
      {
       ...
       this.$refs.table.clearSelection();
        GetRegionalCostQueryList(data).then(res => {
          if(res.success)
          {
    ...
    deletesinge(row){
       this.$confirm('是否删除该报表特殊费用?', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(() => {
                BathDeleteRegionalCost([row.id]).then(res => {
                    if(res.success)
                    {
    ...
    onBatchDelete() {
      ...
      this.$confirm("是否确定删除", "提示", {
        ...
        BathDeleteRegionalCost(arr).then(res => {
          if(res.success){
    ...
    openUpdate(row){
      ...
      this.$refs.EditComp.GetRegionalCostById(row.id);
    },
    openAdd(){
      this.EditComp.show = true
      ...
      this.$confirm("是否确定提交", "提示", {
        ...
        this.bthloading=true;
        SubmitRegionalCost(arr).then(res => {
           this.bthloading=false;
            if(res.result!="" && res.result!==null && res.result !==undefined){
               warnMsg(res.result);
             }
