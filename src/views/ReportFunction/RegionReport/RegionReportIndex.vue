<template>
  <div class="app-container regionreport" v-loading="bthloading">
    <el-form size="mini" @submit.native.prevent label-width="70px">
      <el-row>
        <el-col :span="3">
          <el-form-item label="大区：" prop="" style="position: relative">
            <!-- <el-input
              placeholder="大区"
              style="width: 100%"
...
import {
  RegionInquireProfitStatement,
  ExportBUFile,
} from "api/ReportFunction/RegionReport";
...
export default {
  ...
  methods: {
    ...
    //添加查询combox
    setComBox() {
      GetRegionList({IsAuthority:true}).then((res) => {
    ...
    getTableList() {
      ...
      if(this.search.Region==="" || this.search.Region===null || this.search.Region===undefined){
        warnMsg("请选择大区");
        return;
      }
      ...
      RegionInquireProfitStatement(data)
        .then((res) => {
          if (res.success) {
    ...
    Importdata() {
     ...
     data.IsShowWY=this.search.IsShowWY;
      ...
      //   IsShowWY: 0,
      //   IsMerge: 0,
     var filename="大区报表"+"-"+this.search.Region;
       if (
        this.budyear !== "" &&
        this.budyear !== null &&
        this.budyear !== undefined
     ...
     data.filename=filename;
      data.sheetname="大区报表";
      ExportBUFile(this.TableDataList).then(res=>{
        if(res.success){
           this.bthloading=false;
