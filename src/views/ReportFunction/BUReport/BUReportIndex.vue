<template>
  <div class="app-container bureport" v-loading="bthloading">
    <el-form size="mini" @submit.native.prevent label-width="70px">
      <el-row>
        <el-col :span="3">
          <el-form-item label="BU：" prop="" style="position: relative">
            <!-- <el-input
              placeholder="BU"
              style="width: 100%"
              v-model="search.BU"
        ...
        <el-col :span="3">
         ...
         <!-- <el-col :span="4">
                <el-form-item label="合资公司导入版本："  label-width="150px" >               
        ...
        <el-col :span="3">
          <el-form-item label prop label-width="10px">
            ...
            <el-button
              ...
              @click="Importdata()"
              :loading="bthloading"
              v-if="checkBtnPeimission(BUReportExport.export.permission)"
              >导出</el-button
            >
          </el-form-item>
        </el-col>
    ...
    <div id="out-table" style="height: calc(100% - 40px);">
      ...
      >
        ...
        <el-table-column
          v-for="(item, index) in TableDataList.titles"
          :key="index"
          :label="item.name"
          width="130px"
          align="center"
          ...
          >
            ...
            >
              ...
              >
                <template
                  slot-scope="scope"
                  v-if="item.MaxLevel !== fourtitem.level"
                >
                  <span>{{ scope.row.value[fourtitem.index] }}</span>
                </template>
                   <el-table-column
                      v-for="(fivetitem, fiveindex) in fourtitem.children"
                      :key="fiveindex"
                      :label="fivetitem.name"
                      width="150"
                      align="center"
                      show-overflow-tooltip
                      :v-if="item.MaxLevel === fourtitem.level"
                    >
                      ...
                      >
                        <span>{{ scope.row.value[fivetitem.index] }}</span>
                      </template>
...
import {
  BUInquireProfitStatement,
  ExportBUFile,
} from "api/ReportFunction/BUReport";
import { warnMsg, htmlMsg } from "utils/messageBox";
import { checkBtnPeimission, BUReportExport } from "utils/btnRole";
import clumitem from "./clumitem";
...
export default {
  name: "BUReportIndex",
  components: { treeSelect, tree, "column-item": clumitem },
  mixins: [tableMixin],
  data() {
    return {
      BUReportExport,
  ...
  methods: {
    ...
    //    var x = document.getElementsByClassName("el-table__header")[0].rows[1].cells;
    //   // 将第二列表头单元格的colSpan设为2
    //   x[0].colSpan = 2;
    //    x[1].colSpan = 0;
    //   // 将第三列表头单元格的display设为none
    ...
    getTableList() {
      ...
      if (
        this.sbudyear !== "" &&
        this.sbudyear !== null &&
        this.sbudyear !== undefined
      ) {
       sy = this.sbudyear.getFullYear();
       sm = this.sbudyear.getMonth() + 1;
      } else {
      ...
      if (
        ...
        ty = this.budyear.getFullYear();
        tm = this.budyear.getMonth() + 1;
      } else {
        warnMsg("请选择截止所属时间");
        return;
      }
      ...
      // ) {
      //   var st1=  parseTime(this.submitDateRange[0]);
      //   var st2 = parseTime(this.submitDateRange[1]);
      //   if( st1.getFullYear() != st2.getFullYear())
      //   {
      //      warnMsg("请选择同一年份！");
      //      return;
      //   }
      //   data.Year = st1.getFullYear();
      //   data.Month =st1.getMonth() + 1;
      //   data.SMon = st2.getMonth() + 1;
      ...
      BUInquireProfitStatement(data)
        .then((res) => {
          if (res.success) {
            ...
            //记录
            var startindex = -1;
            this.TableDataList.contents.forEach((ele, index) => {
              if (ele.project.includes("/")) {
                ...
                if (parentname === namelist[0]) {
                  ...
                  if (startindex > -1 && ishb === true) {
                   this.TableDataList.contents[startindex].rowspaln = rows;
                  }
                  ishb = true;
                  rows = 1;
                  startindex = index;
    ...
    Importdata() {
      ...
      data.IsShowWY=this.search.IsShowWY;
      // BU: "",
      //   FinanceBudget: 0,
      //   Version: 1 ,
      //   IsCompare:0,
      //   IsShowWY:0,
      //   IsMerge:0,
      var filename="BU报表"+"-"+this.search.BU;
       if (
        this.budyear !== "" &&
        this.budyear !== null &&
        this.budyear !== undefined
        ...
        filename=filename+"-"+this.budyear.getFullYear()+"-"
         +(this.sbudyear.getMonth()>9?(this.sbudyear.getMonth()+1):("0"+(this.sbudyear.getMonth()+1)))+"至"
        +(this.budyear.getMonth()>9?(this.budyear.getMonth()+1):("0"+(this.budyear.getMonth()+1)));
        
      } 
      filename=filename+"-"+(this.search.FinanceBudget===0?"财务":"预算")
...
// var all= document.getElementById("out-table").outerHTML ;
// //var newhtml=all.replace("<th colspan=\"0\" rowspan=\"3\" class=\"el-table_22_column_1676  is-center   is-leaf\" style=\"display: none;\">\<div class=\"cell\">项目</div></th>","");
    
//      var htnl =
//         "<html><head><meta charset='utf-8' /><style> table,table tr th, table tr td { border:1px solid black;} </style></head><body><div>" +
//        document.getElementById("out-table").outerHTML+
//         "</div></body></html>";
...
//       }
  ...
  created() {
    this.setComBox();
      var dd=new Date();
    var newdd=this.budyear.getFullYear()+'-01-01';
    this.sbudyear=new Date(newdd);
  },
  mounted() {
